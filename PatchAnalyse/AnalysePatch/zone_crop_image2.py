...
# 这是目前采用的方法
def get_video_patch(src,savepath):
    imgInfo = image.shape
    # cnt = 1
    # num = 1
    sub_images = []
    sub_image_num_hight=5
    sub_image_num_width=5
    ...
    for i, img in enumerate(sub_images):
        cv2.imwrite(savepath+'sub_img_' + str(i) + '.jpg', img)
...
if __name__ == '__main__':
    ...
    savepath=currpath+"/Patches/classification{}".format(result1)
    get_video_patch(image,savepath)
